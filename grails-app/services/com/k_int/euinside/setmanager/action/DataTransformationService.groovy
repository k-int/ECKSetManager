...
class DataTransformationService extends ServiceActionBase {
	...
	def process(ProviderSet set) {
		...
		Record.findAllWhere(set : set, live : false, validationStatus : Record.VALIDATION_STATUS_OK, convertedData : null, deleted : false).each() {

			if (it.originalData != null) {			
				// Increment the record processed count
				recordsProcessed++;
				byte [] convertedData = Transform.transformWait1(set.provider.code, it.id.toString(), Format.LIDO, Format.EDM, it.originalData); 
